...
return {

    {
        ...
        config = function()
            ...
            local pum_opts = { silent = true, noremap = true, expr = true, replace_keycodes = false }

            -- 注意：整行一行，不能在中间回车
            keyset(
                "i",
                "<Tab>",
                [[coc#pum#visible() ? coc#pum#next(1) : coc#jumpable() ? "\<C-r>=coc#rpc#request('doKeymap', ['snippets-expand-jump',''])\<CR>" : v:lua.CheckBackspace() ? "\<Tab>" : coc#refresh()]],
                pum_opts
            )
            ...
            for _, o in ipairs(objs) do
                xmap("i" .. o[1], "<Plug>(coc-" .. o[2] .. "-i)")
                vim.keymap.set("o", "i" .. o[1], "<Plug>(coc-" .. o[2] .. "-i)")
                xmap("a" .. o[1], "<Plug>(coc-" .. o[2] .. "-a)")
            ...
            nmap("<leader>ca", ":<C-u>CocList diagnostics<cr>")
            nmap("<leader>ce", ":<C-u>CocList extensions<cr>")
...

    {
        ...
        config = function()
            require("tabout").setup({
                tabkey = "<Tab>",             -- 向前 tabout 使用的键
                backwards_tabkey = "<S-Tab>", -- 向后 tabout 使用的键
                act_as_tab = true,            -- 非成对符号位置时，行为像普通 Tab
                act_as_shift_tab = true,
                enable_backwards = true,
                completion = true, -- 与补全插件（包括 coc.nvim）协作
                ignore_beginning = true,
                exclude = { "TelescopePrompt", "NvimTree", "neo-tree", "alpha" },
            })
        end,
    },
