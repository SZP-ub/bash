...
return {
...

    {
        "junegunn/fzf.vim",
        event = "VeryLazy",
        dependencies = { "junegunn/fzf", "nvim-tree/nvim-web-devicons" }, -- fzf 二进制
        config = function()
            -- 文件查找
        ...
        config = function()
            require("fzf-lua").setup({
                ...
                winopts = {
                    ...
                    height     = 0.75,    -- 窗口高度（占编辑区的百分比）
                    width      = 0.70,    -- 窗口宽度（占编辑区的百分比）
    ...
    --
    --         -- 查找文件
    --         vim.keymap.set('n', '<space>ff', fzf.files, { desc = "fzf-lua: 查找文件" })
    --         -- 全文搜索（单次 grep）
    --         vim.keymap.set('n', '<space>fg', fzf.live_grep, { desc = "fzf-lua: 全文搜索" })
    --         -- 查找头文件（正则过滤 .h 结尾）
    --         vim.keymap.set('n', '<space>fh', function()
    --             fzf.grep({ rg_opts = "--type-add 'header:*.h' --type header" })
    --         end, { desc = "fzf-lua: 查找头文件" })
    --         -- buffer 列表
    --         vim.keymap.set('n', '<space>fb', fzf.buffers, { desc = "fzf-lua: buffer 列表" })
    --         -- 最近文件
    --         vim.keymap.set('n', '<space>fr', fzf.oldfiles, { desc = "fzf-lua: 最近文件" })
    --         -- 查找包含当前文件的文件（#include "xxx"）
    --         vim.keymap.set('n', '<space>fi', function()
    --             local fname = vim.fn.expand('%:t')
    --             fzf.grep({ search = '#include "' .. fname .. '"' })
    --         end, { desc = "fzf-lua: 查找包含当前文件的文件" })
    --         -- 查找 tags（ctags 需先生成）
    --         vim.keymap.set('n', '<space>ft', fzf.tags, { desc = "fzf-lua: 查找 tags" })
    --         -- 查找调用关系（用当前光标单词搜索）
    ...
    -- cscope_maps.nvim 集成
    {
        ...
        opts = {
            ...
            -- prefix = "<space>c",       -- cscope 相关操作的快捷键前缀
        ...
        config = function(_, opts)
            ...
            vim.keymap.set('n', '<space>csb', ':!cscope -Rbqkv<CR>', { desc = "构建/更新 cscope 数据库（项目根目录）" })

            -- 打开调用关系树（stack view）
            vim.keymap.set('n', '<space>csn', function()
            ...
            -- vim.keymap.set('n', '<space>css', "<cmd>Cscope find s <C-R><C-W><CR>", { desc = "查找符号（symbol）" })
            vim.keymap.set('n', '<space>csg', "<cmd>Cscope find g <C-R><C-W><CR>",
                { desc = "查找全局定义（global definition）" })
            vim.keymap.set('n', '<space>csc', "<cmd>Cscope find c <C-R><C-W><CR>", { desc = "查找调用该函数的位置（callers）" })
            vim.keymap.set('n', '<space>csi', '<cmd>Cscope find c <C-R>=expand("<cword>")<CR><CR>',
                { desc = "Cscope: 调用我的函数" })
            vim.keymap.set('n', '<space>cso', '<cmd>Cscope find d <C-R>=expand("<cword>")<CR><CR>',
                { desc = "Cscope: 我调用的函数" })
            vim.keymap.set('n', '<space>cst', "<cmd>Cscope find t <C-R><C-W><CR>", { desc = "查找被该函数调用的位置（callees）" })
            -- vim.keymap.set('n', '<space>cse', "<cmd>Cscope find e <C-R><C-W><CR>", { desc = "查找字符串（text string）" })
            -- vim.keymap.set('n', '<space>csf', "<cmd>Cscope find f <C-R><C-W><CR>", { desc = "查找文件（file）" })
            -- vim.keymap.set('n', '<space>csi', "<cmd>Cscope find i <C-R><C-W><CR>", { desc = "查找包含文件（#include）" })
        end,
    },
